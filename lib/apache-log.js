 ...
 * Copyright 2015 Raimar Falke <i-apache-log-node@rf.risimo.net>
 *
...
var default_custom_remote_ip_header = 'x-forwarded-for';
var custom_remote_ip_header = '';
var output_stream = false;
var prefix = null
var stdoutWrite = function(log) {
    console.log(log);
};
var fsWrite = function(log) {
    if (output_stream === false) {
        open_or_update_file();
    }

    fs.write(output_stream, log + "\n");
};
var write = fsWrite;
...
function get_remote_ip(request) {
    return request.headers[custom_remote_ip_header] ||
        request.headers[default_custom_remote_ip_header] ||
...
function responseEnd(request, responseBodyLen, responseStatusCode) {
    var common_log = sprintf((prefix || '') + '%s - %s [%s] "%s %s %s/%s" %s %s',
    ...
    write(common_log + combined, output_stream);
}
...
function configure(options) {
    var new_format = options.format;
    var new_path = options.path;

    prefix = options.prefix;
    custom_remote_ip_header = options.custom_remote_ip_header;
    if (custom_remote_ip_header) {
        custom_remote_ip_header = custom_remote_ip_header.toLowerCase();
    }
    ...
    output_stream = false;
    write = output_path === null ? stdoutWrite : write;
}
